...
class ExerciseService:
    ...
    async def generate_personalized_exercise(
        ...
        exercise_data = await self.llm_service.generate_exercise(
            user_id=str(user_id),
            language=context.programming_language,
            skill_level=context.skill_level,
            interests=context.learning_goals or context.preferred_topics or "",
            recent_topics=context.previous_topics or [],
            difficulty=context.difficulty_override or "medium",
            estimated_time=30
        )
        ...
        exercise = Exercise(
            ...
            instructions=exercise_data.get("requirements", ""),
            starter_code=None,  # Could be added later
            solution=None,  # Not generated by default
            exercise_type=exercise_type or ExerciseType.ALGORITHM,
            difficulty=self._map_difficulty(difficulty or "medium"),
            programming_language=context.programming_language,
            topics=",".join(exercise_data.get("objectives", [])),
            test_cases=json.dumps(exercise_data.get("test_cases") or []),
            generated_by_ai=True,
            generation_prompt=json.dumps({
                "language": context.programming_language,
                "skill_level": context.skill_level,
                "difficulty": difficulty or "medium",
                "topics": context.preferred_topics
            })
        )
    ...
    async def submit_exercise(
        ...
        # Build exercise description for LLM
        ...
        # Evaluate with LLM
        evaluation = await self.llm_service.evaluate_submission(
            user_id=str(user_id),
        ...
        user_exercise.grade = evaluation["score"]  # LLM returns "score" not "grade"
    ...
    async def request_hint(
        ...
        # Get user for skill level context
        user = await self._get_user(user_id)
        ...
        hint_text = await self.llm_service.generate_hint(
            user_id=str(user_id),
            exercise_description=exercise_description,
            student_code=current_code,
            student_question=context,
            skill_level=user.skill_level or "intermediate",
            hints_count=user_exercise.hints_requested
        )
